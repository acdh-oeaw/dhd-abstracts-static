...
try:
    client.collections["DHd-Abstracts"].delete()
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "DHd-Abstracts",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"https://github.com/acdh-oeaw/dhd-abstracts-static/{record['id']}"
...
print(make_index)
print("done with indexing DHd-Abstracts")

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {"action": "upsert"})
print(make_index)
print("done with cfts-index DHd-Abstracts")
